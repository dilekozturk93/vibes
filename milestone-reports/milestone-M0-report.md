...
> ⚠ **Superseded by [M0-rev](milestone-M0-rev-report.html).** The converter
...
> in [milestone-M0-rev-report](milestone-M0-rev-report.html).
...
- `.gitignore`: fix `.DS_Store` glob (was `.DS_Store/` with trailing slash so
 nothing matched), ignore local IDE / tool state directories, `.vscode/`,
 `**/bin/`, and the exploratory `vibes-example/*.dot` / `*.png` outputs.
...
**M1 :** apply `MxeToFtsConverter` to `eMail/eM_ESGFx.mxe`
