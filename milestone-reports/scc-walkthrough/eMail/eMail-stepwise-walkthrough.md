...
**Size:** 8 states, 16 transitions

FExpressionPreservingProjection keeps only transitions whose feature expression evaluates to true under the chosen configuration. 6 transition(s) were dropped relative to step 1. After projection the graph has 2 strongly-connected component(s); the SCC containing the initial state has 6 state(s). If that count is less than the total state count, the next step removes the unreachable / non-returnable portions.

![Step 2](eMail-step2-projected.png)

## Step 3 — Repaired FTS (initial-state SCC only)

**Size:** 6 states, 12 transitions

InitialSccFilter keeps only the SCC containing the initial state and drops every other state along with the transitions touching them. 2 state(s) and 4 transition(s) were removed. The result is strongly connected by construction and feeds the EulerianBalancer + Hierholzer pipeline in M3 and the coverage generators in M4 / M5 / M6.
